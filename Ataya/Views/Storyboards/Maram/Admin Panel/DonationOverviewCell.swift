...
final class DonationOverviewCell: UITableViewCell {
    ...
    func configure(item: DonationItem) {
        ...
        badgeLabel.text = item.status.displayText
        ...
        // ✅ Image from donations.photoURLs[0]
        let url = item.imageUrl.trimmingCharacters(in: .whitespacesAndNewlines)
        currentImageUrl = url
        itemImageView.image = nil

        guard !url.isEmpty else { return }

        itemImageView.fetchRemoteImage(urlString: url) { [weak self] img in
