...
//  Ataya
//
//  Saves DraftDonation into Firestore as donations/DON-x
//
...
final class DonationDraftSaver {
    ...
    /// Call this AFTER pickup screen is completed (date/time/method/address ready)
    func saveAfterPickup(draft: DraftDonation, completion: @escaping (Error?) -> Void) {
        ...
        // ✅ Normalize strings (trim, empty -> nil, etc.)
        draft.normalizeBeforeSave()
        ...
        if let existingId = draft.id,
           !existingId.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            ...
            // ✅ IMPORTANT: save pickup INSIDE same setData
            data["pickup"] = pickupMap

            print("🟡 SAVING UPDATE donations/\(existingId):", data)
        ...
        reserveNextDonationNumber(db: db) { [weak self] result in
            ...
            case .success(let number):
                ...
                print("🟢 SAVING CREATE donations/\(docId):", data)
    ...
    private func buildPickupDict(draft: DraftDonation) -> [String: Any] {
        ...
        if let a = draft.pickupAddress {
            // لازم يكون عند pickupAddress دالة toFirestoreDict()
            pickup["address"] = a.toFirestoreDict()
        }
