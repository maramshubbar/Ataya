...
//                }
//
//                // ✅ روحي لصفحة Organization Details
...
final class NGOSignUpViewController: UIViewController {
    ...
    @IBAction func signUpTapped(_ sender: UIButton) {
        ...
        guard isTermsChecked else { showAlert(title: "Terms Required", message: "Please accept Terms & Privacy to continue."); return }
        guard !name.isEmpty else { showAlert(title: "Missing NGO Name", message: "Please enter NGO name."); return }
        guard !email.isEmpty else { showAlert(title: "Missing Email", message: "Please enter your email."); return }
        guard !phone.isEmpty else { showAlert(title: "Missing Phone", message: "Please enter your phone number."); return }
        guard password.count >= 6 else { showAlert(title: "Weak Password", message: "Password must be at least 6 characters."); return }
        ...
        Auth.auth().createUser(withEmail: email, password: password) { [weak self] result, error in
            ...
            self.db.collection("users").document(uid).setData(ngoData) { err in
                ...
                // ✅ تروح لصفحة Organization Details
                self.performSegue(withIdentifier: "ngoToOrgDetailsSegue", sender: nil)
            }
        }
    }
    ...
    private func updateSignUpButtonState() {
        ...
        let isValid = isTermsChecked && !name.isEmpty && !email.isEmpty && !phone.isEmpty && !p1.isEmpty && (p1 == p2)
        signUpButton.isEnabled = isValid
        signUpButton.alpha = isValid ? 1.0 : 0.5
    }
    ...
    private func updatePasswordMatchUI() {
        ...
        confirmPasswordTextField.layer.borderColor = (p1 == p2) ? UIColor.systemGreen.cgColor : UIColor.systemRed.cgColor
    }
}
